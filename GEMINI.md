...
- **ASALDESIGN 시스템 준수**: 모든 UI 작업은 `DESIGN.md`와 `DESIGN.json`에 정의된 ASALDESIGN 시스템을 엄격히 따릅니다.
    - **DESIGN.json**: 색상값, 픽셀값 등 정확한 수치 데이터가 필요할 때 참조합니다.
    - **DESIGN.md**: 레이아웃 설계, 위계 설정, 디자인 의도 및 "Do's and Don'ts"를 파악할 때 참조합니다.
...
- [ ] 사용자 계정 관리 기능 구현
- [ ] 링크 관리 CRUD 구현
- [ ] 프로필 설정 및 퍼블리싱 페이지 구현 (기본 프로필 페이지 `my-profile` 구현 완료)
